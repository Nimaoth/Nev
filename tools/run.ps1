...
while ($exitCode -eq 123) {
    ...
    $argString = "-p:editor.prompt-before-quit=true -p:editor.watch-app-config=true -p:editor.watch-user-config=false -p:editor.watch-workspace-config=false $argString --no-wasm"
