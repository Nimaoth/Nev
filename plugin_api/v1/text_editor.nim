...
proc recordCurrentCommand*(editor: TextEditor; registers: WitList[WitString]): void {.
    nodestroy.} =
  ## Records the current command being executed to the given registers.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc hideCompletions*(editor: TextEditor): void {.nodestroy.} =
  ## Hides any open completion popups in the editor.
  var arg0: uint64
  arg0 = editor.id
...
proc scrollToCursor*(editor: TextEditor; behaviour: Option[ScrollBehaviour];
  ...
  ## Scroll the view to bring the cursor into view. The behaviour determines how the cursor is positioned.
  ## `relativePosition` determines the vertical position of the cursor within the viewport (0 = top, 1 = bottom).
  var
    arg0: uint64
    arg1: int8
    arg2: int8
...
proc updateTargetColumn*(editor: TextEditor): void {.nodestroy.} =
  ## Updates the target column (the column the cursor tries to keep when moving up/down).
  var arg0: uint64
  arg0 = editor.id
...
proc getUsage*(editor: TextEditor): WitString {.nodestroy.} =
  ## Returns the usage describing what the editor is used for (e.g command line, search bar, normal etc).
  var
...
proc getRevision*(editor: TextEditor): int32 {.nodestroy.} =
  ## Returns the current revision number of the document.
  var arg0: uint64
  arg0 = editor.id
...
proc setMode*(editor: TextEditor; mode: WitString; exclusive: bool): void {.
    nodestroy.} =
  ## Sets the editor mode. If exclusive is true, all other modes are removed.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc textEditorUndoImported(a0: uint64): void {.
    wasmimport("undo", "nev:plugins/text-editor").}
proc undo*(editor: TextEditor): void {.nodestroy.} =
  ## Undo the last change.
  var arg0: uint64
  arg0 = editor.id
  textEditorUndoImported(arg0)

proc textEditorRedoImported(a0: uint64): void {.
    wasmimport("redo", "nev:plugins/text-editor").}
proc redo*(editor: TextEditor): void {.nodestroy.} =
  ## Redo the last undone change.
  var arg0: uint64
  arg0 = editor.id
  textEditorRedoImported(arg0)

proc textEditorStartTransactionImported(a0: uint64): void {.
    wasmimport("start-transaction", "nev:plugins/text-editor").}
proc startTransaction*(editor: TextEditor): void {.nodestroy.} =
  ## Start a transaction. Every edit until `end-transaction` will be grouped together and undone in one go.
  var arg0: uint64
  arg0 = editor.id
  textEditorStartTransactionImported(arg0)

proc textEditorEndTransactionImported(a0: uint64): void {.
    wasmimport("end-transaction", "nev:plugins/text-editor").}
proc endTransaction*(editor: TextEditor): void {.nodestroy.} =
  ## End the current transaction.
  var arg0: uint64
  arg0 = editor.id
  textEditorEndTransactionImported(arg0)
...
proc copy*(editor: TextEditor; register: WitString; inclusiveEnd: bool): void {.
    nodestroy.} =
  ## Copy the current selection to the specified register. If inclusiveEnd is true, the selection includes the character at the end position.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc paste*(editor: TextEditor; selections: WitList[Selection];
  ...
  ## Paste the contents of the register at the given selections.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc toggleLineComment*(editor: TextEditor): void {.nodestroy.} =
  ## Toggle line comments for the current selection using the language's comment style.
  var arg0: uint64
  arg0 = editor.id
...
proc insertText*(editor: TextEditor; text: WitString; autoIndent: bool): void {.
    nodestroy.} =
  ## Insert text at the current cursor position(s). If auto-indent is true, the text will be auto-indented.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc openSearchBar*(editor: TextEditor; query: WitString; scrollToPreview: bool;
  ...
  ## Open the interactive search bar with the given initial query.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc applyMove*(editor: TextEditor; selection: Selection; move: WitString;
    ...
    nodestroy.} =
  ## Apply a move operation to the selection and return the new selection(s).
  ## The move is a string that specifies the movement (e.g., "word", "line-end").
  ## count specifies how many times to repeat the move, wrap controls whether to wrap around the document.
  var
...
proc multiMove*(editor: TextEditor; selections: WitList[Selection];
    ...
    Selection] {.nodestroy.} =
  ## Apply a move operation to multiple selections at once.
  var
    retArea: array[40, uint8]
    arg0: uint64
    arg1: int32
...
proc evaluateExpressions*(editor: TextEditor; selections: WitList[Selection];
  ...
  ## Evaluates the selected text as expressions and replaces them with the result.
  ## prefix and suffix are added around each selection before evaluating.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc indent*(editor: TextEditor; delta: int32): void {.nodestroy.} =
  ## Indent or unindent the selected lines by the given delta (positive = indent, negative = unindent).
  var
    arg0: uint64
    arg1: int32
  arg0 = editor.id
...
proc getCommandCount*(editor: TextEditor): int32 {.nodestroy.} =
  ## Returns the current command count (used for repeat prefix like "3w").
  var arg0: uint64
  arg0 = editor.id
...
proc setCursorScrollOffset*(editor: TextEditor; cursor: Cursor;
  ...
  ## Sets the vertical scroll offset for a specific cursor position.
  var
    arg0: uint64
    arg1: int32
    arg2: int32
...
proc createAnchors*(editor: TextEditor; selections: WitList[Selection]): WitList[
  ...
  ## Creates anchors at the start and end of each selection. Anchors track a position in the document
  ## even as text is inserted or deleted before them, allowing for persistent selections.
  var
    retArea: array[16, uint8]
    arg0: uint64
    arg1: int32
...
proc resolveAnchors*(editor: TextEditor; anchors: WitList[(Anchor, Anchor)]): WitList[
    Selection] {.nodestroy.} =
  ## Resolves anchors back to selections using the current document state.
  var
    retArea: array[16, uint8]
    arg0: uint64
    arg1: int32
...
proc edit*(editor: TextEditor; selections: WitList[Selection];
    ...
    nodestroy.} =
  ## Edits the document by replacing the selections with the given contents.
  ## Returns the new selections after the edit.
  var
...
proc defineMove*(move: WitString; fun: uint32; data: uint32): void {.nodestroy.} =
  ## Defines a custom move operation. The callback receives the rope, current selections,
  ## count, and includeEol flag, and returns the new selections.
  var
    arg0: int32
    arg1: int32
    arg2: uint32
