...
proc listenEvent*(fun: uint32; data: uint32; id: WitString; pattern: WitString): void {.
    nodestroy.} =
  ## Register a callback function to be called when events matching the pattern occur.
  ## The callback receives the event name and payload (JSON string).
  var
    arg0: uint32
    arg1: uint32
    arg2: int32
...
proc stopListenEvent*(id: WitString; pattern: WitString): void {.nodestroy.} =
  ## Stop listening to events that match the pattern.
  var
    arg0: int32
    arg1: int32
    arg2: int32
