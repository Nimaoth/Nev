...
type
  ...
  LispVal* = ref object
    ...
    of Func:
      name*: string
      evalArgs*: bool = true
      fn*: proc (args: seq[LispVal]): LispVal {.gcsafe, raises: [LispError].}
    of Lambda, Macro:
      params*: seq[string]
      body*: LispVal
      env*: Env
...
proc newNumber*(n: int): LispVal =
  LispVal(kind: Number, num: n.float)

proc newBool*(n: bool): LispVal =
  LispVal(kind: Bool, bol: n)
...
proc newFunc*(name: string, evalArgs: bool, fn: proc(args: seq[LispVal]): LispVal {.gcsafe, raises: [LispError].}): LispVal =
  LispVal(kind: Func, name: name, evalArgs: evalArgs, fn: fn)

proc newFunc*(name: string, fn: proc(args: seq[LispVal]): LispVal {.gcsafe, raises: [LispError].}): LispVal =
  LispVal(kind: Func, name: name, evalArgs: true, fn: fn)

proc newFunc*(fn: proc(args: seq[LispVal]): LispVal {.gcsafe, raises: [LispError].}): LispVal =
  LispVal(kind: Func, evalArgs: true, fn: fn)
