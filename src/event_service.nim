...
proc listen*(self: EventService, id: Id, pattern: string, cb: EventListener) =
  if id == idNone():
...
proc stopListen*(self: EventService, id: Id, pattern: string = "") =
  if id == idNone():
...
proc emit*(self: EventService, event: string, data: string) =
