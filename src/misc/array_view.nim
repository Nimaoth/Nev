...
proc `[]=`*[T](arr: var ArrayView[T], index: int, value: sink T) =
  assert index >= 0
  assert index < arr.len
  arr.data[index] = value

proc first*[T](arr {.byref.}: ArrayView[T]): Option[ptr T] =
  if arr.len > 0:
    result = arr.data[0].addr.some

proc last*[T](arr {.byref.}: ArrayView[T]): Option[ptr T] =
  if arr.len > 0:
    result = arr.data[arr.high].addr.some

proc add*[T](arr: var ArrayView[T], val: sink T) {.nodestroy.} =
  assert arr.len < arr.capacity
  arr.data[arr.len.int] = val.ensureMove
  inc arr.len

proc add*[T](arr: var ArrayView[T], vals: sink ArrayView[T]) {.nodestroy.} =
