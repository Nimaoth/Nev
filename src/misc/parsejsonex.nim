...
from std/parsejson import JsonKindError, JsonParsingError
export parsejson.JsonKindError, parsejson.JsonParsingError
...
type
  ...
  JsonexParser* = object of BaseLexer ## the parser object.
    a*: string
    tok*: TokKind
    kind: JsonEventKind
    err: JsonError
...
proc close*(my: var JsonexParser) {.inline.} =
  ## closes the parser `my` and its associated input stream.
  lexbase.close(my)

proc str*(my: JsonexParser): string {.inline.} =
  ## returns the character data for the events: `jsonInt`, `jsonFloat`,
  ## `jsonString`
  assert(my.kind in {jsonInt, jsonFloat, jsonString})
  return my.a

proc getInt*(my: JsonexParser): BiggestInt {.inline.} =
  ## returns the number for the event: `jsonInt`
  assert(my.kind == jsonInt)
  return parseBiggestInt(my.a)

proc getFloat*(my: JsonexParser): float {.inline.} =
  ## returns the number for the event: `jsonFloat`
  assert(my.kind == jsonFloat)
  return parseFloat(my.a)

proc kind*(my: JsonexParser): JsonEventKind {.inline.} =
  ## returns the current event type for the JSON parser
  return my.kind

proc getColumn*(my: JsonexParser): int {.inline.} =
  ## get the current column the parser has arrived at.
  result = getColNumber(my, my.bufpos)

proc getLine*(my: JsonexParser): int {.inline.} =
  ## get the current line the parser has arrived at.
  result = my.lineNumber

proc getFilename*(my: JsonexParser): string {.inline.} =
  ## get the filename of the file that the parser processes.
  result = my.filename

proc errorMsg*(my: JsonexParser): string =
  ## returns a helpful error message for the event `jsonError`
  assert(my.kind == jsonError)
  result = "$1($2, $3) Error: $4" % [
    my.filename, $getLine(my), $getColumn(my), errorMessages[my.err]]

proc errorMsgExpected*(my: JsonexParser, e: string): string =
...
proc parseString(my: var JsonexParser): TokKind =
  result = tkString
  var pos = my.bufpos + 1
...
proc parseNumber(my: var JsonexParser) =
  var pos = my.bufpos
  if my.buf[pos] == '-':
    add(my.a, '-')
    inc(pos)
...
proc parseName(my: var JsonexParser) =
  var pos = my.bufpos
...
proc getTok*(my: var JsonexParser): TokKind =
  setLen(my.a, 0)
  skip(my) # skip whitespace, comments
  my.tokenStartPos = my.bufpos
...
proc raiseParseErr*(p: JsonexParser, msg: string) {.noinline, noreturn.} =
  ## raises an `EJsonParsingError` exception.
  raise newException(JsonParsingError, errorMsgExpected(p, msg))

proc eat*(p: var JsonexParser, tok: TokKind) =
