...
when defined(useDynlib):
  when defined(mallocImport):
    proc roots(): ptr CellSeq[Cell] {.importc, dynlib: "nev.exe".}
  else:
    when defined(mallocExport):
      var
        roots2 {.threadvar.}: CellSeq[Cell]

      proc roots(): ptr CellSeq[Cell] {.exportc, dynlib.} = roots2.addr
else:
  var roots2 {.threadvar.}: CellSeq[Cell]
  proc roots(): ptr CellSeq[Cell] {.inline.} = roots2.addr
...
when defined(useDynlib):
  when defined(mallocImport):
    proc rootsThreshold(): ptr int {.importc, dynlib: "nev.exe".}
  else:
    when defined(mallocExport):
      var rootsThreshold2 {.threadvar.}: int

      proc rootsThreshold(): ptr int {.exportc, dynlib.} = rootsThreshold2.addr
else:
  var rootsThreshold2 {.threadvar.}: int
  proc rootsThreshold(): ptr int {.inline.} = rootsThreshold2.addr
...
proc collectCycles() =
  ...
  let rootsThreshold = rootsThreshold()
  when not defined(nimStressOrc):
