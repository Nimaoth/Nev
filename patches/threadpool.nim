...
var
  currentPoolSize {.apprtl.}: int
  maxPoolSize {.apprtl.} = MaxThreadPoolSize
  minPoolSize {.apprtl.} = 4
  gSomeReady {.apprtl.}: Semaphore
  readyWorker {.apprtl.}: ptr Worker

# A workaround for recursion deadlock issue
# https://github.com/nim-lang/Nim/issues/4597
var
  numSlavesLock {.apprtl.}: Lock
  numSlavesRunning {.apprtl, guard: numSlavesLock.}: int
  numSlavesWaiting {.apprtl, guard: numSlavesLock.}: int
  isSlaveVar {.threadvar.}: bool

proc isSlave(): ptr bool {.apprtl.} = isSlaveVar.addr
...
var
  workers {.apprtl.}: array[MaxThreadPoolSize, Thread[ptr Worker]]
  workersData {.apprtl.}: array[MaxThreadPoolSize, Worker]

  distinguished {.apprtl.}: array[MaxDistinguishedThread, Thread[ptr Worker]]
  distinguishedData {.apprtl.}: array[MaxDistinguishedThread, Worker]
...
var
  state {.apprtl.}: ThreadPoolState
  stateLock {.apprtl.}: Lock
