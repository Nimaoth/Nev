...
# PATCH: replaced KeyError with KeyDefect, make everything gcsafe and raises: []
type KeyDefect* = object of Defect
...
proc raiseKeyError[T](key: T) {.noinline, noreturn.} =
  when compiles($key):
    raise newException(KeyDefect, "key not found: " & $key)
  else:
    raise newException(KeyDefect, "key not found")

template get(t, key): untyped =
  ## retrieves the value at `t[key]`. The value can be modified.
  ## If `key` is not in `t`, the `KeyDefect` exception is raised.
  mixin rawGet
  var hc: Hash
