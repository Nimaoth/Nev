...
proc handleViewRender(id: int32, data: uint32) {.cdecl.} =
  ...
  try:
    ...
    buildCommands(renderCommandEncoder):
      ...
      drawText("test_plugin_v1 version " & $version, rect(size * 0.5, vec2()), color(0.5, 0.5, 1, 1), 0.UINodeFlags)
...
defineCommand(ws"test-command-3",
  active = false,
  ...
  proc(data: uint32, args: WitString): WitString {.cdecl.} =
    try:
      echo &"[guest] test-command-3 {data} '{args}'"
      inc target
    except CatchableError as e:
      echo &"[guest] err: {e.msg}"
    return ws""

defineCommand(ws"test-command-4",
  active = false,
  ...
  proc(data: uint32, args: WitString): WitString {.cdecl.} =
    try:
      echo &"[guest] test-command-4 {data} '{args}'"
      dec target
    except CatchableError as e:
      echo &"[guest] err: {e.msg}"
    return ws""

defineCommand(ws"open-custom-view-2",
  active = false,
