...
- `v0` - The most recent version of the API on the main branch. This can have regular breaking changes.
...
- `v1`, `v2`, `v3` ... - The stable version of the API. If the current stable version is `v3` and a new version of Nev gets released, the current state of `v0` is copied and becomes the new stable version `v4`. New development continues on `v0`, with only bug fixes being applied to stable versions.
