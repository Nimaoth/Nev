#use layout text_editor_component register command_service
...
proc createTerminalView*(self: TerminalService, stdin: Arc[BaseChannel], stdout: Arc[BaseChannel], options: CreateTerminalOptions, id: Id = idNone()): View = terminalServiceCreateTerminalViewChannel(self, stdin, stdout, options, id)
proc createTerminalView*(self: TerminalService, command: string, options: CreateTerminalOptions, id: Id = idNone()): View = terminalServiceCreateTerminalView(self, command, options, id)

when not implModule:
  static:
    echo "DONT Build terminal implementation"
...
when implModule:
  ...
  proc createTexture(self: TerminalServiceImpl, sixel: sink Sixel) =
    try:
  ...
  proc cleanupUnusedSixels(self: TerminalServiceImpl) {.async.} =
    while true:
  ...
  proc handleOutputChannel(self: TerminalServiceImpl, terminal: Terminal) {.async.} =
    ...
    var callbacks = VTermScreenCallbacks(
      ...
      settermprop: (proc(prop: VTermProp; val: ptr VTermValue; user: pointer): cint {.cdecl.} =
        ...
        of VTERM_PROP_CURSORSHAPE:
          ...
          of VTERM_PROP_CURSORSHAPE_BLOCK: types_impl.CursorShape.Block
          of VTERM_PROP_CURSORSHAPE_UNDERLINE: types_impl.CursorShape.Underline
          of VTERM_PROP_CURSORSHAPE_BAR_LEFT: types_impl.CursorShape.BarLeft
          else: types_impl.CursorShape.Block
  ...
  when defined(enableLibssh):
    ...
    proc createSshSession(self: TerminalServiceImpl, terminal: Terminal, stdin: Arc[BaseChannel], stdout: Arc[BaseChannel], command: string, args: seq[string] = @[], options: SshOptions): Future[void] {.async: (raises: []).} =
      try:
  ...
  proc createTerminal*(self: TerminalServiceImpl, width: int, height: int, command: string, args: seq[string] = @[], autoRunCommand: string = "", createPty: bool = true, kittyPathPrefix: string = "", ssh: Option[SshOptions]): Terminal {.raises: [IOError, OSError, ResourceExhaustedError].} =
    if ssh.isSome:
    ...
    when defined(windows):
      ...
      if CreatePipe(addr(inputReadHandle), addr(inputWriteHandle), sa.addr, 0) == 0:
        raiseOSError(osLastError(), "Failed to create pipes")
  ...
  proc activateTerminalView*(self: TerminalView) =
    if self.active:
      return
    self.active = true
    self.markDirtyBase()

  proc deactivateTerminalView*(self: TerminalView) =
    if not self.active:
      return
    self.active = false
    self.markDirtyBase()

  template withActiveView*(self: TerminalServiceImpl, view: TerminalView, body: untyped): untyped =
    block:
  ...
  proc desc*(self: TerminalView): string = &"TerminalView"
  proc kind*(self: TerminalView): string = "terminal"
  proc display*(self: TerminalView): string = &"term://{self.terminal.command} - {self.terminal.group}"
  proc saveState*(self: TerminalView): JsonNode =
  ...
  proc handleThemeChanged(self: TerminalServiceImpl, theme: Theme) =
    for view in self.terminals.values:
  ...
  proc handlePaste(self: TerminalServiceImpl, view: TerminalView, text: string) =
  ...
  proc updateModeEventHandlers(self: TerminalServiceImpl, view: TerminalView) =
    if view.mode.len == 0:
  ...
  proc terminalServiceCreateTerminalView(self: TerminalService, command: string, options: CreateTerminalOptions, id: Id = idNone()): View =
    let self = self.TerminalServiceImpl
    try:
  ...
  proc terminalServiceCreateTerminalViewChannel(self: TerminalService, stdin: Arc[BaseChannel], stdout: Arc[BaseChannel], options: CreateTerminalOptions, id: Id = idNone()): View =
    let self = self.TerminalServiceImpl
    try:
  ...
  proc getActiveView(self: TerminalServiceImpl): Option[TerminalView] =
  ...
  proc createTerminal*(self: TerminalServiceImpl, command: string = "", options: CreateTerminalOptions = CreateTerminalOptions()) =
  ...
  proc isIdle(self: TerminalServiceImpl, terminal: Terminal): bool =
  ...
  proc sendTerminalInput*(self: TerminalServiceImpl, input: string, noKitty: bool = false) =
    if self.getActiveView().getSome(view):
  ...
  proc terminalPreviewerDeinit*(self: TerminalPreviewer) =
    self[] = default(typeof(self[]))

  proc terminalPreviewerDelayPreview*(self: TerminalPreviewer) =
    discard

  proc terminalPreviewerPreviewItem*(self: TerminalPreviewer, item: FinderItem): View =
  ...
  proc selectTerminal*(self: TerminalServiceImpl, preview: bool = true, scaleX: float = 0.9, scaleY: float = 0.9, previewScale: float = 0.6) =
    proc getItems(): seq[FinderItem] {.gcsafe, raises: [].} =
      let allViews = self.layout.allViews()
    ...
    builder.scope = "terminals".some
    builder.previewScale = previewScale
    builder.scaleX = scaleX
    builder.scaleY = scaleY
    builder.finder = finder.some

    var previewer: TerminalPreviewer = nil
    if preview:
      previewer = newTerminalPreviewer(self.services, self)
      builder.previewer = previewer.Previewer.some

    builder.handleItemConfirmed = proc(popup: ISelectorPopup, item: FinderItem): bool =
      if previewer != nil and previewer.view != nil:
        previewer.view.isInPreview = false
      let id = item.data.parseInt.catch:
        log lvlError, fmt"Failed to parse editor id from data '{item}'"
        return true
